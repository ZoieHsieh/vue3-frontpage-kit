      ...
      <div class="selected-items">
        <div class="selected-item" v-for="(item, index) in selectedItems" :key="item.value">
          ...
          <div class="item-controls">
            <button @click="moveItem(index, -1)" :disabled="index === 0" class="control-btn">左移</button>
            <button @click="moveItem(index, 1)" :disabled="index === selectedItems.length - 1" class="control-btn">
              右移
            </button>
          </div>
        </div>
...
const selectedItems = computed(() =>
  selectedCategories.value.map((id) => depthOneOptions.value.find((item) => item.value === id))
)
...
const submitCategories = async () => {
  const payload = {
    ...
    companyId: 1,
    productCategories: selectedCategories.value.map((id, index) => ({
...
const moveItem = (index: number, direction: number) => {
  ...
  if (newIndex < 0 || newIndex >= selectedCategories.value.length) return
  const items = [...selectedCategories.value]
